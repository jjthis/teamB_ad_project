...
class Display:
    ...
    def prep_piece_picture(self):
        ...
        self.img_item_time = pygame.transform.scale(self.img_item_time,
                                                    (JANGI_BOARD_CELL_PIXELS, JANGI_BOARD_CELL_PIXELS))
        self.img_item_mulligan = pygame.transform.scale(self.img_item_mulligan,
                                                        (JANGI_BOARD_CELL_PIXELS, JANGI_BOARD_CELL_PIXELS))
...
class Jangi:
    ...
    def jang_move(self, src_i, src_j, target_i, target_j):
        if abs((target_i - src_i) * (target_j - src_j)) == 1:
            return False
        elif (abs(target_i - src_i) == 1 and target_j - src_j == 0) or (
                abs(target_i - src_i) == 0 and abs(target_j - src_j) == 1):
            return True
        else:
            return False
