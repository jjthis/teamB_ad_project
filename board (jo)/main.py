...
while jangi.running:
    ...
    if jangi.input.mouse_clicked:
        ...
        and
         jangi.input.mx <= JANGI_BOARD_PADDING+4*JANGI_BOARD_CELL_PIXELS) and (JANGI_BOARD_PADDING <= jangi.input.my 
         and
           ...
           #((jangi.input.mx >= 10 and jangi.input.mx <= 150) or (jangi.input.my >= 110 and jangi.input.my <= 550))):
            ...
            if not isValid:
                ...
                if not jangi.input.is_src_set:
                    ...
                    and
                    jangi.input.mx <= JANGI_BOARD_PADDING+3*JANGI_BOARD_CELL_PIXELS) and (JANGI_BOARD_PADDING <= jangi.input.my 
                    and
                    jangi.input.my <= JANGI_BOARD_PADDING+4*JANGI_BOARD_CELL_PIXELS))): 
            ...
            if jangi.input.is_src_set and jangi.input.is_target_set and porocheck and porocheck2:
                ...
                continue
            #말움직임
            elif jangi.input.is_src_set and jangi.input.is_target_set and jangi.is_alreadyIn(src_i, src_j, target_i, target_j):
    ...
    for i in range(4): # me
        rect = (JANGI_BOARD_PADDING-JANGI_BOARD_CELL_PIXELS, i*JANGI_BOARD_CELL_PIXELS + JANGI_BOARD_PADDING, JANGI_BOARD_CELL_PIXELS, JANGI_BOARD_CELL_PIXELS)
    ...
    for i in range(4): # you
        rect = (JANGI_BOARD_PADDING+3*JANGI_BOARD_CELL_PIXELS, i*JANGI_BOARD_CELL_PIXELS + JANGI_BOARD_PADDING, JANGI_BOARD_CELL_PIXELS, JANGI_BOARD_CELL_PIXELS)
        pygame.draw.rect(jangi.display.SURFACE, (214,198,182), rect, 0)
    ...
    for N in range(4):
        for M in range(2):
            ...
            if cell_team == ME:
                jangi.display.SURFACE.blit(jangi.display.me_piece_list[sprite_num], 
                (10+JANGI_BOARD_PADDING-JANGI_BOARD_CELL_PIXELS, N*JANGI_BOARD_CELL_PIXELS+JANGI_BOARD_PADDING+10))
            elif cell_team == YOU:
                jangi.display.SURFACE.blit(jangi.display.you_piece_list[sprite_num], 
                (3*JANGI_BOARD_CELL_PIXELS+JANGI_BOARD_PADDING+10, N*JANGI_BOARD_CELL_PIXELS+JANGI_BOARD_PADDING+10))            
